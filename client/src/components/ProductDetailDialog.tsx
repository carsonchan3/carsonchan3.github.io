...
type Shield205Tier = "builder" | "certified";

export function getShield205Variant(product: ProductDetail, tier: Shield205Tier) {
  const matchesTier = (variant: ProductVariant) => `${variant.label} ${variant.model} ${variant.name}`.toLowerCase();
  const pattern = tier === "builder" ? /pnp|plug/ : /rtf|ready/;
  return product.variants.find((variant) => pattern.test(matchesTier(variant))) ?? product.variants[tier === "builder" ? 1 : 0] ?? product.variants[0];
}
...
function Shield205Detail({ product,   selectedVariant,
  selectedTier,
  onTierChange, onAddToCart }: {
  product: ProductDetail;
  selectedVariant: ProductVariant;
  selectedTier: Shield205Tier;
  onTierChange: (tier: Shield205Tier) => void;
  onAddToCart: (variant: ProductVariant, family: ProductDetail) => void;
}) {
  ...
  const tier = shield205Content.tiers[selectedTier];
  const certified = selectedTier === "certified";
  const image = selectedVariant.image;

  return (
    <div data-testid="tops-shield-205-premium-detail" className="space-y-7">
      <div className="grid gap-7 md:grid-cols-[0.95fr_1.05fr] md:items-stretch">
        <div className="relative flex min-h-64 items-center justify-center overflow-hidden rounded-2xl border border-white/10 bg-[#0B1419] p-6 sm:min-h-80">
          ...
          <img
            ...
            className="relative z-10 max-h-72 w-full object-contain drop-shadow-[0_20px_35px_rgba(0,0,0,0.45)]"
          />
          <span className="absolute bottom-4 left-4 z-20 inline-flex items-center gap-2 rounded-full border border-accent/30 bg-black/45 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-accent">
            <Gauge size={13} /> {translate("205 mm platform")}
          </span>
        </div>
        ...
        <div className="flex flex-col justify-center">
          ...
          <h2 data-testid="product-detail-title" className="mt-3 text-3xl font-semibold leading-tight text-white sm:text-4xl">{translate(shield205Content.title)}</h2>
          <p data-testid="product-detail-description" className="mt-4 text-sm leading-7 text-white/70 sm:text-base">{translate(shield205Content.pitch)}</p>

          <div data-testid="tops-shield-205-tier-options" className="mt-6 grid gap-3 sm:grid-cols-2">
            {(Object.keys(shield205Content.tiers) as Shield205Tier[]).map((option) => {
              const optionVariant = getShield205Variant(product, option);
              if (!optionVariant) return null;
              const selected = option === selectedTier;
              const isRecommended = option === "certified";
              return (
                <button
                  type="button"
                  key={option}
                  data-testid={`tops-shield-205-tier-${option}`}
                  aria-pressed={selected}
                  onClick={() => onTierChange(option)}
                  className={`relative rounded-2xl border p-4 text-left transition-colors ${selected ? "border-accent bg-accent/10" : "border-white/10 bg-black/20 hover:border-white/30"}`}
                >
                  ...
                  <span className="block text-sm font-semibold text-white">{translate(shield205Content.tiers[option].label)}</span>
                  <span className="mt-1 block min-h-10 text-xs leading-5 text-white/55">{translate(shield205Content.tiers[option].subtitle)}</span>
          ...
          <div className="mt-5 rounded-2xl border border-white/10 bg-white/[0.03] p-4">
            <div className="flex items-start gap-3">
              <div className="mt-0.5 rounded-full bg-accent/15 p-2 text-accent"><Wrench size={16} /></div>
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.14em] text-white/45">{translate(tier.label)}</p>
                <p className="mt-2 text-sm leading-6 text-white/75">{translate(tier.subtitle)}</p>
                <ul className="mt-3 space-y-2">
                  {tier.features.map((feature) => <li key={feature} className="flex gap-2 text-xs leading-5 text-white/65"><Check size={15} className="mt-0.5 shrink-0 text-accent" />{translate(feature)}</li>)}
                </ul>
              </div>
            </div>
          </div>
          ...
          {certified ? (
            <div data-testid="vli-care-badge" className="mt-4 flex items-center gap-3 rounded-2xl border border-accent/35 bg-accent/10 p-4 text-accent">
              <ShieldCheck size={24} className="shrink-0" />
              <div><p className="text-sm font-bold">{translate("Includes 1-Year VLI CARE")}</p><p className="mt-1 text-xs leading-5 text-white/65">{translate("Coverage for heavy collision damage, water damage, and rapid replacements.")}</p></div>
            </div>
          ) : null}
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <section data-testid="tops-shield-205-specifications" className="rounded-2xl border border-white/10 bg-black/20 p-5">
          <div className="mb-4 flex items-center gap-2"><Gauge size={17} className="text-accent" /><h3 className="text-lg font-semibold text-white">{translate("Technical Specifications")}</h3></div>
          <dl className="space-y-3">{shield205Content.specifications.map(([label, value]) => <div key={label} className="grid grid-cols-[0.8fr_1.2fr] gap-3 border-t border-white/10 pt-3 text-sm"><dt className="text-white/45">{translate(label)}</dt><dd className="text-right text-white/75">{translate(value)}</dd></div>)}</dl>
        </section>
        <section data-testid="tops-shield-205-in-the-box" className="rounded-2xl border border-white/10 bg-black/20 p-5">
          <div className="mb-4 flex items-center gap-2"><Box size={17} className="text-accent" /><h3 className="text-lg font-semibold text-white">{translate("In the Box")}</h3></div>
          <ul className="space-y-3">{[...shield205Content.inTheBox, ...(certified ? ["1x VLI CARE Activation Code"] : [])].map((item) => <li key={item} className="flex gap-2 text-sm leading-6 text-white/75"><Check size={15} className="mt-1 shrink-0 text-accent" />{translate(item)}</li>)}</ul>
        </section>
      </div>
    </div>
  );
...
export default function ProductDetailDialog({ product, onOpenChange, onAddToCart }: ProductDetailDialogProps) {
  ...
  const [selectedShieldTier, setSelectedShieldTier] = useState<Shield205Tier>("certified");
  const isShield205 = product?.familyId === "tops-shield-205";

  useEffect(() => {
    setSelectedShieldTier("certified");
    setSelectedVariantId(product?.variants[0]?.sourceId ?? "");
  }, [product?.familyId]);

  const selectedVariant = useMemo(() => {
    if (!product) return undefined;
    if (product.familyId === "tops-shield-205") {
      return getShield205Variant(product, selectedShieldTier);
    }
    return product.variants.find((variant) => variant.sourceId === selectedVariantId) ?? product.variants[0];
  }, [product, selectedShieldTier, selectedVariantId]);
    ...
    <Dialog open={Boolean(product)} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[92vh] overflow-y-auto border-white/10 bg-[#1C1D20] text-white sm:max-w-5xl">
        {product && selectedVariant ? (
          isShield205 ? (
            <Shield205Detail product={product}               selectedVariant={selectedVariant}
              selectedTier={selectedShieldTier}
              onTierChange={setSelectedShieldTier} onAddToCart={onAddToCart} />
          ) : (
            <>
              <DialogHeader>
