...
export const pricingFamilyPresentation = "deep-ink-service-family";
...
export default function RefereePricingConfigurator() {
  ...
  return (
    <section id="pricing" className={`${pricingVisibilityClass} border-y border-white/10 bg-[#071117] py-14 text-white md:py-24`}>
      <div className="container">
        <div data-reveal className="reveal-up mx-auto mb-9 max-w-3xl text-center">
          ...
          <p className="mt-4 leading-7 text-white/70">Start with the delivery outcome that best fits your venue, event team, and level of decision support.</p>
        </div>

        <aside data-testid="event-fit-panel" data-reveal className="reveal-up mx-auto mb-8 max-w-5xl rounded-[1.5rem] border border-white/15 bg-[#0B1419] p-5 shadow-[0_18px_42px_rgba(0,0,0,0.18)] sm:p-7">
          <div className="grid gap-5 lg:grid-cols-[0.86fr_1.14fr] lg:items-end"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-accent">Event fit</p><h3 className="mt-3 text-2xl font-semibold tracking-tight text-white sm:text-3xl">Scope your event before the quote.</h3></div><p className="text-sm leading-7 text-white/70">Share your venue, cage count, match format, programme schedule, and required level of delivery support. Your selected package remains part of the request.</p></div>
          <div className="mt-6 grid gap-2 border-t border-white/10 pt-5 sm:grid-cols-2 lg:grid-cols-4">{eventFitPlanningInputs.map((input, index) => <div key={input} className="flex items-center gap-2 text-sm font-semibold text-white/80"><span className="font-mono text-xs text-accent">0{index + 1}</span>{input}</div>)}</div>
        </aside>
        ...
        <div data-presentation={pricingFamilyPresentation} className="rounded-[1.75rem] border border-white/10 bg-[#0B1419] p-3 shadow-[0_20px_55px_rgba(0,0,0,0.22)] sm:p-4">
          <div className="grid gap-3 lg:grid-cols-3">
            {pricingTiers.map((tier) => {
              ...
              return (
                <button key={tier.id} type="button" data-testid={`pricing-tier-${tier.id}`} data-detail-presentation={pricingCardDetailPresentation} data-card-presentation={pricingTierCardPresentation} aria-pressed={selected} onClick={() => setSelectedTier(tier.id)} className={`group relative min-h-0 overflow-hidden rounded-[1.25rem] border p-5 text-left transition-all duration-200 ${selected ? "border-accent bg-[#123B3C] shadow-[0_0_0_1px_rgba(64,224,208,0.24)]" : "border-white/10 bg-[#101D22] hover:border-white/25 hover:bg-[#13252A]"}`}>
                  ...
                  <div className="mt-6 flex items-end gap-2"><p data-testid={`pricing-price-${tier.id}`} className="text-2xl font-bold tracking-tight text-accent">{tier.price}</p><p className="pb-0.5 text-sm text-white/60">{tier.priceUnit}</p></div>
                  <ul className="mt-5 space-y-2 border-t border-white/10 pt-4">
                    {tier.features.slice(0, 3).map((feature) => <li key={feature} className="flex items-start gap-2 text-sm leading-5 text-white/80"><CheckCircle2 aria-hidden="true" size={16} className="mt-0.5 shrink-0 text-accent" />{feature}</li>)}
                  </ul>
                  <p className={`mt-6 text-sm font-semibold ${selected ? "text-white" : "text-white/65 group-hover:text-white"}`}>{selected ? "Selected support" : `Select ${tier.name}`}</p>
                </button>
              );
            })}
          </div>
        ...
        <details data-reveal className="reveal-up group mt-6 overflow-hidden rounded-xl border border-white/10 bg-[#0B1419]" style={{ transitionDelay: "70ms" }}>
