        ...
        <section id="technical-confidence" data-testid="technical-confidence" className="velocity-section border-b border-white/10 bg-[#171C1D]">
          <div className="container">
            ...
            <div data-testid="event-scale-evidence-panels" data-presentation={eventScaleEvidencePanelPresentation} data-mobile-columns={eventScaleFeatureTilePresentation.mobileColumns} data-desktop-columns={eventScaleFeatureTilePresentation.desktopColumns} data-tile-aspect-ratio={eventScaleFeatureTilePresentation.tileAspectRatio} data-visibility={eventScaleFeatureTilePresentation.visibility} className="mt-10 grid grid-cols-2 gap-3 sm:gap-4 xl:grid-cols-4 xl:gap-5">
              <article data-event-scale-panel data-feature-tile data-tile-index="01" data-reveal className="reveal-up aspect-square overflow-hidden rounded-xl border border-white/10 bg-[radial-gradient(circle_at_70%_20%,rgba(64,224,208,0.18),transparent_0_34%),#0B1419] p-4 sm:p-5" style={{ transitionDelay: "0ms" }}>
                <p className="text-[10px] font-semibold tracking-[0.2em] text-accent sm:text-xs">01 · DECISION DATA</p><div className="mt-4 grid grid-cols-2 gap-2 sm:gap-3">{proofPoints.map((point) => <div key={point.value} className="border-l border-white/15 pl-2 sm:pl-3"><p className="text-lg font-semibold tracking-[-0.05em] text-white sm:text-2xl">{point.value}</p><p className="mt-1 text-[9px] leading-3 text-white/55 sm:text-[10px] sm:leading-4">{point.label}</p></div>)}</div><div className="mt-3 flex items-center gap-2 border-t border-white/10 pt-3">{smartRefereeMedia.ruleSupportLogos.map((logo) => <img key={logo.id} src={logo.src} alt={logo.alt} loading="lazy" decoding="async" className="h-4 min-w-0 flex-1 object-contain sm:h-5" />)}</div><p className="sr-only">{technicalConfidence.description} {technicalConfidence.referenceCaption}</p>
              </article>
              <article data-event-scale-panel data-feature-tile data-tile-index="02" data-reveal className="reveal-up relative aspect-square overflow-hidden rounded-xl border border-white/10 bg-[#0B1419] p-4 sm:p-5" style={{ transitionDelay: "70ms" }}>
                <img src={smartRefereeMedia.stickers} alt="Circular passive marker stickers for competition drones" className="absolute inset-0 h-full w-full object-cover opacity-35" /><div className="absolute inset-0 bg-gradient-to-t from-[#071014] via-[#071014]/70 to-transparent" /><div className="relative flex h-full flex-col"><p className="text-[10px] font-semibold tracking-[0.2em] text-accent sm:text-xs">02 · TRACKING SETUP</p><h3 className="mt-auto text-lg font-semibold tracking-tight text-white sm:text-2xl">{technicalConfidence.markerTitle}</h3><p className="mt-2 hidden text-[10px] leading-4 text-white/70 sm:block">{technicalConfidence.markerDescription}</p><p className="sr-only">{technicalConfidence.markerDescription}</p></div>
              </article>
              <div data-testid="technical-evidence-panels" className="contents">
                <article data-event-scale-panel data-technical-evidence-panel data-feature-tile data-tile-index="03" data-reveal className="reveal-up relative aspect-square overflow-hidden rounded-xl border border-white/10 bg-black p-4 sm:p-5" style={{ transitionDelay: "140ms" }}>
                  <video data-testid="continuous-calibration-video" src={smartRefereeMedia.continuousCalibrationVideo} autoPlay={continuousCalibrationVideoPresentation.autoPlay} muted={continuousCalibrationVideoPresentation.muted} loop={continuousCalibrationVideoPresentation.loop} controls={continuousCalibrationVideoPresentation.controls} playsInline={continuousCalibrationVideoPresentation.playsInline} preload={continuousCalibrationVideoPresentation.preload} className="absolute inset-0 h-full w-full object-cover opacity-45">Your browser does not support embedded video.</video><div className="absolute inset-0 bg-gradient-to-t from-[#071014] via-[#071014]/60 to-transparent" /><div className="relative flex h-full flex-col"><p className="text-[10px] font-semibold tracking-[0.2em] text-accent sm:text-xs">03 · CONTINUOUS CALIBRATION</p><h3 className="mt-auto text-lg font-semibold tracking-tight text-white sm:text-2xl">{technicalConfidence.continuousCalibrationTitle}</h3><p className="mt-2 hidden text-[10px] leading-4 text-white/70 sm:block">{technicalConfidence.continuousCalibrationDescription}</p><p className="sr-only">{technicalConfidence.continuousCalibrationDescription}</p></div>
                </article>
                <article data-event-scale-panel data-technical-evidence-panel data-pitch-video-panel data-feature-tile data-tile-index="04" data-reveal className="reveal-up aspect-square overflow-hidden rounded-xl border border-white/10 bg-[#0B1419] p-4 sm:p-5" style={{ transitionDelay: "210ms" }}>
                  <p className="text-[10px] font-semibold tracking-[0.2em] text-accent sm:text-xs">04 · FLEX 13 ILLUMINATION</p><h3 className="mt-2 text-sm font-semibold tracking-tight text-white sm:text-lg">{technicalConfidence.infraredTitle}</h3><p className="sr-only">{technicalConfidence.infraredDescription}</p><div className="mt-3 border-t border-white/10 pt-3"><h3 className="text-sm font-semibold tracking-tight text-white sm:text-base">{technicalConfidence.pitchVideoTitle}</h3><video data-testid="flex13-system-video" src={smartRefereeMedia.precisionVideo} poster={smartRefereeMedia.precisionPoster} aria-label="Drone Sports Referee Pitch video" autoPlay={flex13SystemVideoPresentation.autoPlay} muted={flex13SystemVideoPresentation.muted} loop={flex13SystemVideoPresentation.loop} controls={flex13SystemVideoPresentation.controls} controlsList={flex13SystemVideoPresentation.controlsList} disablePictureInPicture={flex13SystemVideoPresentation.disablePictureInPicture} playsInline={flex13SystemVideoPresentation.playsInline} preload={flex13SystemVideoPresentation.preload} onContextMenu={(event) => event.preventDefault()} className="mt-2 aspect-video w-full rounded-lg bg-black object-contain">Your browser does not support embedded video.</video></div>
                </article>
              </div>
            </div>
          </div>
