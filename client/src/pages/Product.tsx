...
export const eventScaleTileDetailInteraction = {
  ...
  technicalCards: "static-no-duplicate-copy",
...
function EventScaleTile({ id, label, detail, expanded, onToggle, className, technicalEvidence = false, interactive = true, children }: { id: string; label: string; detail: ReactNode; expanded: boolean; onToggle: () => void; className: string; technicalEvidence?: boolean; interactive?: boolean; children: ReactNode }) {
  const detailId = `event-scale-tile-${id}-detail`;
  return <article data-event-scale-panel data-feature-tile data-tile-index={id} data-technical-evidence-panel={technicalEvidence ? "true" : undefined} data-technical-detail={interactive ? "interactive" : "static"} data-reveal className={`group relative aspect-square overflow-hidden rounded-2xl border border-white/10 shadow-[0_18px_45px_rgba(0,0,0,0.22)] ${className}`}>
