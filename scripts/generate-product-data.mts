...
function parseDetail(source: string, language: Language, file: string): Detail | undefined {
  ...
  const required = ["platformLabel", "premiumTitle", "careTitle", "careDescription", "specificationsTitle", "inTheBoxTitle", "specifications", "inTheBox"];
...
const detailType = `{ platformLabel: string; premiumTitle: string; careTitle: string; careDescription: string; specificationsTitle: string; inTheBoxTitle: string; tiers: Record<string, { label: string; subtitle: string; features: string[] }>; specifications: Array<[string, string]>; inTheBox: string[] }`;
